...
if recruiter:
    ...
    for i in range(1, 21):
        try:
            user, created = User.objects.get_or_create(
                username=f'anacandi{i}', 
                defaults={'email': f'anacandi{i}@example.com'}
            )
            if created:
            ...
            c, created = Candidate.objects.get_or_create(
                user=user, 
                defaults={
                    'full_name': f'Analytics Candidate {i}',
                    'email': user.email,
    ...
    # Ensure jobs from screenshot exist to give them pipeline data
    jobs = []
    ...
    for j in jobs:
        for c in random.sample(candidates, random.randint(8, 15)):
            try:
                app, created = Application.objects.get_or_create(
                    ...
                    defaults={
                        'status': random.choice([
                            'APPLIED', 'RESUME_SCREENING', 'RESUME_SELECTED', 'ROUND_1_PASSED', 
                            'ROUND_2_PASSED', 'ROUND_3_PASSED', 'OFFER_ACCEPTED', 'REJECTED'
                        ])
                    }
                )
                if created:
                ...
                pass
    print("Sample analytics data populated!")
else:
