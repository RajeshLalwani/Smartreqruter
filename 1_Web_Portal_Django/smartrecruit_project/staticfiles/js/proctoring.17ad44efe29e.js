...
class ProctoringSystem {
    constructor(config) {
        ...
        if (!this.config.logUrl || !this.config.applicationId) {
            console.error("Proctoring System: Missing logUrl or applicationId");
        }
    ...
    startVisionHeartbeat() {
        console.log("SecureSight: Vision Heartbeat Started");
        ...
        setInterval(async () => {
            ...
            try {
                ...
                console.error("Heartbeat detection error:", e);
            }
    ...
    async sendHeartbeat(faceCount, stability) {
        try {
            ...
            if (data.status === 'error') {
                console.warn("SecureSight Alert:", data.message);
